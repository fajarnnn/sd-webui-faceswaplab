...
import gradio as gr
# Import FaceSwapLab checkpoints
import importlib.util, sys, os

# --- Faceswap fallback loader ---
import glob

def list_faceswap_checkpoints():
    folder = os.path.join(os.getcwd(), "models", "faceswaplab", "faces")
    ckpts = []
    if os.path.isdir(folder):
        ckpts = glob.glob(os.path.join(folder, "*.onnx")) \
              + glob.glob(os.path.join(folder, "*.pkl")) \
              + glob.glob(os.path.join(folder, "*.safetensors"))
    return [os.path.basename(x) for x in ckpts]

def apply_faceswap_ckpt(p, x, xs):
    import modules.shared as shared
    if x not in (None, "", "None"):
        # Simpan pilihan ke shared.opts (biar bisa dibaca faceswaplab.py)
        shared.opts.data["faceswaplab_face_checkpoint"] = x
...
axis_options.append(
    AxisOption(
        "[Faceswap] Face Checkpoint",
        str,
        apply_faceswap_ckpt,
        choices=lambda: list_faceswap_checkpoints()
    )
)
