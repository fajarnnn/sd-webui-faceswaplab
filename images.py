...
def image_grid(imgs, batch_size=1, rows=None):
    ...
    grid_background_color = ImageColor.getcolor(
        opts.grid_background_color, 'RGB')
    grid = Image.new('RGB', size=(
        params.cols * w, params.rows * h), color=grid_background_color)

    for i, img in enumerate(params.imgs):
        img_w, img_h = img.size
        w_offset, h_offset = 0 if img_w == w else (
            w - img_w) // 2, 0 if img_h == h else (h - img_h) // 2
        grid.paste(img, box=(i % params.cols * w + w_offset,
                   i // params.cols * h + h_offset))
...
def draw_grid_annotations(im, width, height, hor_texts, ver_texts, margin=0):
    ...
    # pastikan semua jadi string
    # hor_texts = [str(x) for x in hor_texts]
    # ver_texts = [str(x) for x in ver_texts]
    ...
    def draw_texts(drawing, draw_x, draw_y, lines, initial_fnt, initial_fontsize):
        for line in lines:
            ...
            # pastikan string
            text_line = getattr(line, "text", str(line))
            is_active = getattr(line, "is_active", False)

            # resize font kalau teks kepanjangan
            while drawing.textlength(text_line, font=fnt) > initial_fontsize and fontsize > 0:
                fontsize -= 1
            ...
            drawing.multiline_text(
                ...
                text_line,  # pastikan ini string
                font=fnt,
                fill=color_active if is_active else color_inactive,
                anchor="mm",
                align="center"
            )


            # coret kalau tidak aktif
            if not is_active:
                drawing.line(
    ...
    pad_left = 0 if sum([sum([len(getattr(line, "text", str(line)))
                        for line in lines]) for lines in ver_texts]) == 0 else width * 3 // 4

    cols = im.width // width
    rows = im.height // height
    # potong atau isi dengan string kosong biar sesuai jumlah kolom

    if len(hor_texts) != cols:
        # potong atau isi dengan string kosong biar sesuai jumlah kolom
        if len(hor_texts) > cols:
            hor_texts = hor_texts[:cols]
        else:
            hor_texts += [""] * (cols - len(hor_texts))

    # assert cols == len(hor_texts), f'bad number of horizontal texts: {len(hor_texts)}; must be {cols}'
    if len(ver_texts) != rows:
        if len(ver_texts) > rows:
            ver_texts = ver_texts[:rows]
        else:
            ver_texts += [""] * (rows - len(ver_texts))

    calc_img = Image.new("RGB", (1, 1), color_background)
    calc_d = ImageDraw.Draw(calc_img)

    for texts, allowed_width in zip(hor_texts + ver_texts, [width] * len(hor_texts) + [pad_left] * len(ver_texts)):
    # pastikan selalu list of GridAnnotation
        if isinstance(texts, str):
            items = [GridAnnotation(texts, True)]
            texts = []
        else:
            items = list(texts)
            texts.clear()
        ...
        for line in items:
            wrapped = wrap(calc_d, getattr(line, "text", str(line)), fnt, allowed_width)
            for x in wrapped:
                texts.append(GridAnnotation(x, getattr(line, "is_active", False)))

        for line in texts:
            if hasattr(calc_d, "multilinebbox"):
                bbox = calc_d.multilinebbox((0, 0), getattr(
                    line, "text", str(line)), font=fnt)
            else:
                bbox = calc_d.textbbox((0, 0), getattr(
                    line, "text", str(line)), font=fnt)
            line.size = (bbox[2] - bbox[0], bbox[3] - bbox[1])
            initial_fontsize = allowed_width

    hor_text_heights = [sum(
        [line.size[1] + line_spacing for line in lines]) - line_spacing for lines in hor_texts]
    ver_text_heights = [sum([line.size[1] + line_spacing for line in lines]) -
                        line_spacing * len(lines) for lines in ver_texts]

    pad_top = 0 if sum(hor_text_heights) == 0 else max(
        hor_text_heights) + line_spacing * 2

    result = Image.new("RGB", (im.width + pad_left + margin * (cols-1),
                       im.height + pad_top + margin * (rows-1)), color_background)

    for row in range(rows):
        for col in range(cols):
            cell = im.crop((width * col, height * row,
                           width * (col+1), height * (row+1)))
            result.paste(cell, (pad_left + (width + margin) *
                         col, pad_top + (height + margin) * row))
    ...
    for row in range(rows):
        x = pad_left / 2
        y = pad_top + (height + margin) * row + height / \
            2 - ver_text_heights[row] / 2
...
def draw_prompt_matrix(im, width, height, all_prompts, margin=0):
    ...
    hor_texts = [[GridAnnotation(x, is_active=pos & (1 << i) != 0) for i, x in enumerate(
        prompts_horiz)] for pos in range(1 << len(prompts_horiz))]
    ver_texts = [[GridAnnotation(x, is_active=pos & (1 << i) != 0) for i, x in enumerate(
        prompts_vert)] for pos in range(1 << len(prompts_vert))]
...
def resize_image(resize_mode, im, width, height, upscaler_name=None):
    ...
    def resize(im, w, h):
        ...
        if scale > 1.0:
            upscalers = [
                x for x in shared.sd_upscalers if x.name == upscaler_name]
            if len(upscalers) == 0:
...
def get_sampler_scheduler(p, sampler):
    ...
    if hasattr(p, 'scheduler') and hasattr(p, 'sampler_name'):
        if sampler:
            sampler_scheduler = get_sampler_scheduler_str(
...
class FilenameGenerator:
        ...
        for style in shared.prompt_styles.get_style_prompts(self.p.styles):
            if style:
                for part in style.split("{prompt}"):
                    prompt_no_style = prompt_no_style.replace(
                        part, "").replace(", ,", ",").strip().strip(',')

                prompt_no_style = prompt_no_style.replace(
                    style, "").strip().strip(',').strip()
    ...
    def datetime(self, *args):
        ...
        time_format = args[0] if (
            args and args[0] != "") else self.default_time_format
        try:
...
def save_image_with_geninfo(image, geninfo, filename, extension=None, existing_pnginfo=None, pnginfo_section_name='parameters'):
    ...
    if extension.lower() == '.png':
        ...
        image.save(filename, format=image_format,
                   quality=opts.jpeg_quality, pnginfo=pnginfo_data)
        ...
        image.save(filename, format=image_format,
                   quality=opts.jpeg_quality, lossless=opts.webp_lossless)
        ...
        image.save(filename, format=image_format,
                   quality=opts.jpeg_quality, exif=exif_bytes)
...
def save_image(image, path, basename, seed=None, prompt=None, extension='png', info=None, short_filename=False, no_prompt=False, grid=False, pnginfo_section_name='parameters', p=None, existing_info=None, forced_filename=None, suffix="", save_to_dirs=None):
    ...
    if save_to_dirs:
        dirname = namegen.apply(
            opts.directories_filename_pattern or "[prompt_words]").lstrip(' ').rstrip('\\ /')
    ...
    if forced_filename is None:
        ...
        if add_number:
            ...
            for i in range(500):
                ...
                fullfn = os.path.join(
                    path, f"{fn}{file_decoration}.{extension}")
    ...
    def _atomically_save_image(image_to_save, filename_without_extension, extension):
        ...
        save_image_with_geninfo(image_to_save, info, temp_file_path, extension,
                                existing_pnginfo=params.pnginfo, pnginfo_section_name=pnginfo_section_name)
    ...
    if hasattr(os, 'statvfs'):
        ...
        fullfn_without_extension = fullfn_without_extension[:max_name_len - max(
            4, len(extension))]
    ...
    if opts.export_for_4chan and (oversize or os.stat(fullfn).st_size > opts.img_downscale_threshold * 1024 * 1024):
        ...
        if oversize and ratio > 1:
            resize_to = round(opts.target_side_length), round(
                image.height * opts.target_side_length / image.width)
        elif oversize:
            resize_to = round(image.width * opts.target_side_length /
                              image.height), round(opts.target_side_length)
...
def read_info_from_image(image: Image.Image) -> tuple[str | None, dict]:
    ...
    if items.get("Software", None) == "NovelAI":
        try:
            ...
            sampler = sd_samplers.samplers_map.get(
                json_info["sampler"], "Euler a")
